...
import com.tu.suanfa.pl.JG2;
import com.tu.util.StringUtil;

public class D19_tz_fengkuang {
	...
	static Map<Integer, Double> map = new LinkedHashMap<Integer, Double>();
	static Map<Integer, Double> map1 = new LinkedHashMap<Integer, Double>();
	...
	static HttpGet get = new HttpGet("http://www.game3799.com/Crazy28/index");
	...
	public static void tz() throws Exception {
		...
		formParams1.clear();
			...
			synchronized(Loading19D.httpClient){
				...
				resList.clear();
				List<String> sss = StringUtil.getRegexStrs(htmlPage, "class=\"js-ball b-fk28\">(\\d+)<");
				for(String s:sss){
				...
				if(time==null||time.length()<=0){
				       Thread.sleep(5000);
						...
						if(time==null||time.length()<=0){
							Thread.sleep(3000);
							response = Loading19D.httpClient.execute(new HttpGet("http://www.game3799.com/Crazy28/index?jdfwkey=agwm02"));
							entity = response.getEntity();
							htmlPage = EntityUtils.toString(entity);
							time = StringUtil.getRegexStr(htmlPage,
									"J_jcTime\" data-lasttime=\"(\\d+)");
...
//				sleepTime = 5;
	...
	public static void postData() throws Exception {
	    ...
	    if((getTime()>7)&&(errorNum>0||CelueControl.checks())){
	        HttpPost post1 = new HttpPost("http://www.game3799.com/Crazy28/Bet");
	...
	public static void setJg() throws Exception {
		HttpPost post = new HttpPost("http://www.game3799.com/Crazy28/BetCurRate");
		RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(
		    8000).setConnectionRequestTimeout(8000).setSocketTimeout(8000)
				.build();
...
//			 FanBeiPL_1.getList(resList, listjg, true);
//            for(int q:listjg){
//                listjg_1.add(q);
////                listjg_1.add(27-q);
//            }
...
//	          GM3.getList(resList, listjg);
			  JG2.getList(listjg, listjg_2, Integer.parseInt(currentNumber),resList,haswin);
	            for(int q:listjg){
	                listjg_1.add(q);
	            }
	            listjg.clear();
...
//            if(listjg_2.size()<1){
                HttpGet getJg = new HttpGet("http://www.game3799.com/crazy28/trend");
                ...
                EntityUtils.consume(response.getEntity());
//            }else{
//                int temc = Integer.parseInt(currentNumber);
//                for(int i=0;i<=27;i++){
//                    if(i==temc){
//                        listjg_2.set(i, 1);  
//                    }else{
//                        listjg_2.set(i, listjg_2.get(i)+1);  
//                    }
//                }
//            }
	}

...
//           for(int j=0;j<=10;j++){
//               arr[j] = new Random().nextInt(27);
//               System.out.print(arr[j]+",");
//           }
//           System.out.println(":"+gm(arr,18));
//        }
    ...
    public static void getList(List<Integer> resList,List<Integer> listjg) {
         getList1(resList,listjg);
    }
    
    public static void getList1(List<Integer> resList,List<Integer> listjg) {
        for(int i=4;i<resList.size();i++){
            double arr[] = new double[i];
            for(int j=1;j<=i;j++){
                arr[arr.length-j] = resList.get(j);
            }
            double tem = gm(arr,resList.get(0));
            int a = (int) Math.ceil(tem);
            int b = (int) Math.floor(tem);
            if(a>=0&&a<=27){
                listjg.add(a); 
                if(a!=27){
                    listjg.add(a+1);   
